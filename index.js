...
// Only ONE notification ever fires: after every retry attempt has been
// exhausted and we're giving up. No per-attempt "retrying..." toasts, so
// there's nothing to stack up, on a refresh or otherwise.
        ...
        while (true) {
            ...
            const reason = networkError
                ...
                : shouldRetryBody
                    ? 'matched error keyword in body'
                    : `HTTP ${response.status}`;
            log(`Failed (${reason}). Retry ${attempt}/${settings.maxRetries} in ${Math.round(delay)}ms.`); // console only — no toast
    ...
    <div class="retry-on-error-settings">
        <div class="inline-drawer">
            ...
            <div class="inline-drawer-content">
                ...
                finally gives up. Check the browser console for a per-attempt log if you want to
                see retries happening in real time.</small>
            </div>
        </div>
    </div>`;
